...
import { lookupHighlights } from "@/lib/story/highlights-lookup";
...
function jsonError(
  ...
  const body: HighlightsLookupResult = { status: "error", code, message };
...
export async function POST(req: NextRequest) {
  ...
  const result = await lookupHighlights(usernameInput);
